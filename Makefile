...
PORT ?= /dev/ttyUSB1
# Programmer type (required for flashing). Prefer the value from include/last_build.make
# but allow callers to override via command line: `make flash PROG=<programmer> PORT=/dev/tty...`.
PROG ?=
PROG_BAUD ?= 115200

# Nano Every touch/reset timing (seconds). Can be overridden on make command line.
TOUCH_DELAY ?= 1
POST_DELAY  ?= 1
...
KNOWN_GOALS := all download-core platform_build clean flash gdb
...
.PHONY: download-core platform_build generate-serial-config interactive flash_interactive
...
# `generate-compile-commands` target removed — compile_commands.json is no longer generated by this repository.
...
flash_interactive:
	@python3 tools/interactive_flash.py
...
flash: $(TARGET).hex
	...
	# Run both Arduino-style touch-reset and DTR toggle to improve bootloader entry
	@echo "Running touch-reset and DTR toggle on $(PORT): touch=$(TOUCH_DELAY)s post=$(POST_DELAY)s"
	python3 tools/touch_reset.py "$(PORT)" $(TOUCH_DELAY) $(POST_DELAY) || true
	python3 tools/toggle_dtr.py "$(PORT)" 100 || true
